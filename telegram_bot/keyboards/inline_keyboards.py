...
from telegram_bot.keyboards.calback_factories import (
    AssistantCallbackFactory,
    DocumentsCallbackFactory,
)

LEXICON_BUT = {"backward": "<<", "forward": ">>"}
...
def crete_inline_keyboard_assistants(assistant_repository: MongoAssistantRepositoryORM, user_tg_id):
    ...
    for assistant in assistants_list:
        kp_builder.button(
            ...
            callback_data=AssistantCallbackFactory(assistant_id=assistant.assistant_id).pack(),
        )

    if document_repository.check_docs_in_user(user_tg_id):
        kp_builder.button(text="История запросов", callback_data="my_docs")
...
def crete_inline_keyboard_payed():
    kp_builder: InlineKeyboardBuilder = InlineKeyboardBuilder()
    kp_builder.button(text="Оплатил", callback_data="payed")
    kp_builder.adjust(1)
    return kp_builder.as_markup()
...
async def crete_inline_keyboard_show_docks(user_tg_id):
    ...
    for key, value in documents.items():
        kp_builder.button(
            ...
            callback_data=DocumentsCallbackFactory(document_date=key).pack(),
        )

    kp_builder.button(text="Назад", callback_data="base")

    kp_builder.adjust(1)
    return kp_builder.as_markup()


async def create_inline_keyboard_show_docs(user_tg_id, docs_per_page=5, current_page=1):
    documents = await document_repository.get_all_users_docs(tg_id=user_tg_id)
    ...
    total_pages = len(sorted_docs) // docs_per_page + (len(sorted_docs) % docs_per_page > 0)
    ...
    if not len(sorted_docs) < docs_per_page:
        ...
        current_page_info = InlineKeyboardButton(text=f"Стр. {current_page} из {total_pages}", callback_data="none")
        kp_builder.row(back_button, forward_button, width=2)
        kp_builder.row(current_page_info, width=1)

        back_button = InlineKeyboardButton(text="главное меню", callback_data="base")
...
def crete_inline_keyboard_back_from_docks():
    ...
    kp_builder.button(text="Назад", callback_data="my_docs")
