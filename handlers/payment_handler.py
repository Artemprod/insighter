...
async def referral_payment_command(callback: CallbackQuery, bot: Bot, tariff_repository: TariffRepoORM):
    ...
    if callback.message.text:
        await callback.message.edit_text(text=REFERRAL_MESSAGE, reply_markup=keyboard)
    else:
        await callback.message.answer(text=REFERRAL_MESSAGE, reply_markup=keyboard)
...
async def process_buy_command(callback: CallbackQuery, bot: Bot, tariff_repository: TariffRepoORM):
    ...
    await bot.send_invoice(
        ...
        prices=[LabeledPrice(label=tariffs["base"].label, amount=tariffs["base"].price * 100)],
...
async def process_successful_payment(
    ...
    try:
        ...
        await user_balance_repo.add_user_time_balance(tg_id=message.from_user.id, time=tariff.minutes * 60)
    except Exception as e:
