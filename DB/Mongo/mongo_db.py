...
class MongoAssistantRepositoryORM:
    ...
    def create_new_assistants(self, assistant: Assistant):
        ...
        try:
            assistant.save()
            insighter_logger.info("Сохранено")
            return as_id
        except Exception:
            insighter_logger.info("что то пошло не так при создании нового асистента ")
            return False
    ...
    def update_assistant_fild(assistant_id, assistant_fild, new_value):
        ...
        try:
            ...
            insighter_logger.info("что то пошло не так при изменение нового поля ")
            return False
    ...
    def delete_assistant(assistant_id: str):
        ...
        try:
            ...
            insighter_logger.info("Удалено")
        except Exception as e:
            insighter_logger.info("Ошибка удаления", e)
...
class MongoUserRepoORM:
    ...
    def add_attempts(tg_id, new_value):
        ...
        try:
            ...
            insighter_logger.exception(
                f"Ошибка в обновлении попыток {user.tg_username}, {user.name}, {user.tg_id}: {e}"
            )
    ...
    async def delete_one_attempt(tg_id):
        ...
        if user.attempts != 0:
            ...
            try:
                ...
                insighter_logger.exception(
                    f"Ошибка в вычитаниее попыток {user.tg_username}, {user.name}, {user.tg_id}: {e}"
                )
    ...
    @staticmethod
    async def add_assistant_call_to_user(user_tg_id, assistant_name):
...
class UserDocsRepoORM:
    ...
    @staticmethod
    async def add_meta_information(tg_id, doc_id, info: dict):
        user: User = User.objects(tg_id=tg_id).get()
        doc = user.documents.get(doc_id)
        if doc is None:
            raise ValueError(f"Документ с ID {doc_id} не найден")
    ...
    @staticmethod
    async def update_user_page(tg_id: int, current_page: int):
        user: User = User.objects(tg_id=tg_id).get()
...
class UserBalanceRepoORM:
    ...
    @staticmethod
    async def add_user_time_balance(tg_id, time):
        user: User = User.objects(tg_id=tg_id).get()
    ...
    @staticmethod
    async def update_time_balance(tg_id, time_to_subtract):
        user: User = User.objects(tg_id=tg_id).get()
    ...
    @staticmethod
    async def add_user_money_balance(tg_id, money):
        user: User = User.objects(tg_id=tg_id).get()
...
class TariffRepoORM:
    ...
    @staticmethod
    async def add_new_tariff(tariff_name, label, price, minutes, currency):
        tariff = Tariff(
...
class TransactionRepoORM:
    ...
    async def save_transaction(
        ...
        try:
            transaction = await self.create_transaction_from_successful_payment(
                tg_user_id=tg_user_id,
                name=name,
                amount=amount,
                tariff=tariff,
