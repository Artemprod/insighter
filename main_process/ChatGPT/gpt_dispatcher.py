...
class TextTokenizer:
    ...
    async def measure_text_capacity(self, text: str) -> int:
        ...
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as pool:
            try:
                text_size = await loop.run_in_executor(pool, self.num_tokens_from_string, text)
    ...
    async def measure_characters_in_tokens(self, tokens_count: int) -> int:
        ...
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as pool:
            try:
                characters = await loop.run_in_executor(pool, self.tokens_to_characters_gpt4_russian, tokens_count)
...
class RequestGPTWorker:
    ...
    async def load_assistant(self, assistant_id) -> Assistant:
        """
    ...
    async def create_one_system_message(self, system_prompt: str, additional_system_information: str) -> GPTMessage:
        prompt = await self.insert_additional_information_if_exists(
            prompt=system_prompt, information=additional_system_information
        )
    ...
    async def create_one_user_message(
        ...
        message = GPTMessage(
            role=GPTRole.USER,
            content=f' {prompt}. Вот текст записи: -> "{income_text}" ',
        )
        return message
...
class OneRequestGPTWorker(RequestGPTWorker):
    async def make_gpt_request(
        ...
        try:
            response = await gpt_client.complete(
                messages=[user_message],
                system_message=system_massage,
            )
            insighter_logger.info("successful gpt api request ")
            return response
...
class BigDataGPTWorker(RequestGPTWorker):
    ...
    async def make_gpt_request(
        ...
        task = asyncio.create_task(self.__make_long_text(string=income_text, assistant_in_work=assistant))
        try:
            result = await task
    ...
    async def split_text(self, string):
    ...
    async def __make_long_text(self, string, assistant_in_work: AssistantInWork):
        ...
        try:
            result = await loop.run_in_executor(
                ...
                partial(map_reduce_chain.run, split_docs),
            )
            return result
        except Exception as e:
...
class GPTDispatcher(IGPTDispatcher):
    ...
    async def compile_request(
        ...
        try:
            ...
            result = await request_method.make_gpt_request(
                assistant_id=assistant_id,
                income_text=income_text,
                additional_system_information=additional_system_information,
                additional_user_information=additional_user_information,
...
class GPTDispatcherOnlyLonghain(IGPTDispatcher):
    ...
    async def compile_request(
        ...
        """
        try:
            result = await self.__long_request_gpt.make_gpt_request(
                assistant_id=assistant_id,
                income_text=income_text,
                additional_system_information=additional_system_information,
                additional_user_information=additional_user_information,
