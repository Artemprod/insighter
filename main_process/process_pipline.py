...
class ProcesQueuePipline:
                ...
                if invoked_text:
                    ...
                    try:
                        ...
                        insighter_logger.exception(
                            e,
                            "cant save recognized text in piplene ",
                            self.__dict__,
                        )
                        await data.fsm_bot_state.set_state(FSMSummaryFromAudioScenario.load_file)
                        await data.telegram_bot.send_message(
    ...
    async def run_process(self):
        ...
        gen_text_tasks = self.create_tasks(
            10,
            self.generate_summary_answer,
            queue_pipeline.text_gen_answer_queue,
