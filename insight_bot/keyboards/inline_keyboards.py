...
from insiht_bot_container import doc_repo
from keyboards.calback_factories import AssistantCallbackFactory, DocumentsCallbackFactory


def crete_inline_keyboard_assistants(assistant_repo: MongoAssistantRepositoryORM, user_tg_id):
    ...
    if doc_repo.check_docs_in_user(user_tg_id):
        kp_builder.button(
...
async def crete_inline_keyboard_show_docks(user_tg_id):
    documents = await doc_repo.get_all_users_docs(tg_id=user_tg_id)
