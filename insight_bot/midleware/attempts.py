...
class CheckAttemptsMiddleware(BaseMiddleware):
    def __init__(self, user_repo: MongoUserRepoORM):
        super().__init__()
    ...
    async def __call__(self, handler: Callable[[TelegramObject, Dict[str, Any]], Awaitable[Any]], event: TelegramObject, data: Dict[str, Any]) -> Any:
        ...
        if not await self.user_repo.check_user_in_db(tg_id=user_id):
            return await handler(event, data)

        attempts = await self.user_repo.get_user_attempts(tg_id=user_id)
