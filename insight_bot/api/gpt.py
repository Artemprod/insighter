...
class GPTAPIrequest:
    ...
    async def conversation(self, user_message, max_tokens_response=1000):
        ...
        return response
    async def total_summary(self, text):
        prompt_text = [{"role": "system", "content": LEXICON_ASSISTANTS["total_summary"]["assistant_prompt"]},
                       {"role": "user", "content": f'{LEXICON_ASSISTANTS["total_summary"]["user_prompt"]} + "\n", {text}'}
                       ]
        m_tokens = await self.num_tokens_from_messages(messages=prompt_text)
        response = await self.send_request_to_chatgpt(prompt_text,
                                                      max_tokens=GPTAPIrequest.MODEL_GPT_4_LIMIT-m_tokens)

        return response
    # async def make_long_text(self, string, max_tokens):
    #     text_parts = await self.split_text_into_parts(string, max_tokens - 600)
    #     context = ""
    #     total_text = ""
    #     for part in text_parts:
    #         prompt_text = [{"role": "system", "content": LEXICON_ASSISTANTS["secretary"]["assistant_prompt"]},
    #                        {"role": "user", "content": f'{LEXICON_ASSISTANTS["secretary"]["user_prompt"]} + "\n", {context + part}'}
    #                        ]
    #         m_tokens = self.num_tokens_from_messages(messages=prompt_text)
    #         response = await self.send_request_to_chatgpt(prompt_text,
    #                                                       max_tokens=GPTAPIrequest.MODEL_GPT_4_LIMIT - m_tokens)
    #         total_text += response
    #         context = part[-500:]
